...
page_title: "kestra_worker_queue Resource - terraform-provider-kestra"
subcategory: ""
description: |-
...
- `queue_id` (String) The Worker Queue identifier (RFC 1123 label: lowercase alphanumerics and hyphens, must start and end with an alphanumeric, max 64 chars). Used as the routing identity; immutable.
- `tags` (Set of String) The canonical tag set of the Worker Queue (each tag is an RFC 1123 label). Must not be empty.
